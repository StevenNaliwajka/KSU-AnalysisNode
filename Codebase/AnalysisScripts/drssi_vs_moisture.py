...
def drssi_vs_moisture():
    ...
    loader.load_data("TVWSScenario", 1, {"DRSSI", "Date (Mon/Day/Year)", "Time (Hour:Min:Sec)"})
    ...
    rssi_df["drssi"] = pd.to_numeric(rssi_df["drssi"], errors='coerce').fillna(0)
    ...
    ax1.plot(merged_df["datetime"], merged_df["drssi_norm"], 'r-', label='DRSSI (Normalized)')
