...
def plot_rssi_comparison(tvws_path, calculated_csv):
    ...
    calc_df = calc_df[calc_df["datetime"] <= cutoff_date]  # <-- filter by cutoff
    ...
    for col in [urssi_col, drssi_col, "calculated_rssi"]:
        if col and col in merged.columns:
            merged = merged[(merged[col] <= -70) | (merged[col] > 0)]

    # --- Filter RSSI values between 0 and -40 dBm ---
    for col in [urssi_col, drssi_col, "calculated_rssi"]:
        if col and col in merged.columns:
            merged = merged[(merged[col] >= -90) | (merged[col] < -100)]
    ...
    for depth in merged["depth"].unique():
        ...
        if urssi_col:
            plt.scatter(depth_data["datetime"], depth_data[urssi_col], label=f"URSSI {depth}", alpha=0.6, s=10, c=colors.get(depth, {}).get("urssi", "gray"))
        if drssi_col:
            plt.scatter(depth_data["datetime"], depth_data[drssi_col], label=f"DRSSI {depth}", alpha=0.6, s=10, c=colors.get(depth, {}).get("drssi", "lightgray"))
    plt.scatter(merged["datetime"], merged["calculated_rssi"], label="Calculated RSSI", s=15, c="black")
    plt.xlabel("Time")
    plt.ylabel("RSSI (dBm)")
    plt.title("All URSSI & DRSSI vs Calculated RSSI")
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.show()
    ...
    for depth in merged["depth"].unique():
        for col, label in [(urssi_col, "URSSI"), (drssi_col, "DRSSI")]:
            if col:
                depth_data = merged[merged["depth"] == depth]
                plt.figure(figsize=(12, 6))
                plt.scatter(depth_data["datetime"], depth_data[col], label=f"{label} {depth}", alpha=0.6, s=10, c=colors.get(depth, {}).get(label.lower(), "gray"))
                plt.scatter(depth_data["datetime"], depth_data["calculated_rssi"], label="Calculated RSSI", s=15, c="black")
                plt.xlabel("Time")
                plt.ylabel("RSSI (dBm)")
                plt.title(f"{label} {depth} vs Calculated RSSI")
                plt.legend()
                plt.grid()
                plt.tight_layout()
                plt.show()
